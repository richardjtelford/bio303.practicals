...
#' @importFrom ggplot2 ggplot aes geom_point geom_line coord_equal
...
plot_grid <- function(x, choices = 1:2){
  ...
  ggplot(x, aes(x = .data[[dim_names[1]]], y = .data[[dim_names[2]]])) +
    geom_line(aes(group = .data$col, colour = I(.data$col_colour)), 
              show.legend = FALSE) +
    geom_line(aes(group = .data$row, colour = I(.data$row_colour)), 
              show.legend = FALSE) +
    geom_point() +
    coord_equal() 
}
